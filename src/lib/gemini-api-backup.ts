...
const MIN_TIME_BETWEEN_CALLS = 500;
...
export const generateAIResponse = async (userMessage: string): Promise<string> => {
  if (!GOOGLE_AI_API_KEY || GOOGLE_AI_API_KEY.includes('your_google_ai_api_key_here') || !GOOGLE_AI_API_KEY.startsWith('AIza')) {
    throw new Error('Google AI API Key tidak valid. Silakan update API Key dengan yang benar dari https://aistudio.google.com/app/apikey');
  }

  const MAX_RETRIES_PER_MODEL = 2;
  const RETRY_DELAY = 1000;
  const MODEL_SWITCH_DELAY = 500;
  ...
  for (let modelIndex = 0; modelIndex < MODELS.length; modelIndex++) {
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES_PER_MODEL; attempt++) {
      try {
        ...
        const url = `https://generativelanguage.googleapis.com/${API_VERSION}/models/${currentModel}:generateContent?key=${GOOGLE_AI_API_KEY}`;
        ...
        if (response.status === 403 || response.status === 402 || response.status === 400) {
          const errorData = await response.json();
          lastError = new Error(`${errorData.error?.message || 'Quota exceeded'} for ${currentModel}`);
  ...
  const errorMessage = `Semua model gagal (${failedModels.join(', ')}). Error terakhir: ${lastError?.message || 'Unknown error'}. Periksa API Key atau kuota Anda di https://aistudio.google.com/`;
...
export const testAPIConnection = async (): Promise<boolean> => {
  try {
    const response = await fetch(
      `https://generativelanguage.googleapis.com/${API_VERSION}/models?key=${GOOGLE_AI_API_KEY}`
    );

    if (response.ok) {
      const data = await response.json();
      console.log("✅ API Key valid. Model tersedia:", data.models?.map((m: any) => m.name));
      return true;
    } else {
      console.error("❌ API Key tidak valid atau tidak ada akses");
      return false;
    }
  } catch (error) {
