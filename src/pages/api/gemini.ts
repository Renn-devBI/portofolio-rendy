...
"Maaf, saya hanya bisa membantu seputar layanan percetakan Printology. Apakah Anda ingin mengetahui cara pembayaran, bantuan pemesanan, atau info layanan cetak tertentu?"`;
  ...
  const RETRY_DELAY = 1000; // Ditingkatkan dari 1000ms ke 1500ms
    ...
    for (let attempt = 1; attempt <= MAX_RETRIES_PER_MODEL; attempt++) {
      try {
        ...
        const response = await fetch(
          `https://generativelanguage.googleapis.com/v1beta/models/${currentModel}:generateContent?key=${GOOGLE_AI_API_KEY}`,
          {
            method: "POST",
            headers: {
        ...
        // Handle 503 Overload - langsung ganti model
  ...
  return res.status(500).json({ 
    error: "Semua model AI sedang mengalami kendala. Silakan coba beberapa saat lagi.",
    shouldRetry: true 
  });
}